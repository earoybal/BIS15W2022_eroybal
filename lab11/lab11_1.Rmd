...
homerange %>%
  ...
  geom_histogram(bar = 40, alpha = 0.5, color = 'gray10', fill = 'gray 45') +
  labs(title = 'Distribution of log10.hra values')
```
...
homerange %>%
  ggplot(aes(x = log10.hra)) +
  geom_density(size = 1, color = 'forest green', fill = 'maroon') +
  labs(title = 'Density of log10.hra Values')
```
...
homerange %>%
  ggplot(aes(x = log10.hra)) +
  geom_histogram(aes(y = ..density..), fill = 'sky blue', color = 'gray 34', alpha = .5, bin = 10) +
  geom_density(color = 'yellow', size = 1.25) +
  labs(title = 'Distribution of Homerange Sizes')
```
...
head(range_cat_homerange)

range_cat_homerange %>%
  ggplot(aes(x = range_category, fill = class)) +
  geom_bar(position = 'dodge', color = 'lavender', alpha = .25) +
  labs(title = 'range of taxonomic classes in mass sizes')
```
...
range_cat_homerange %>%
  ...
  geom_boxplot() +
  labs(title = 'range of mass for small animals in each class')
```
