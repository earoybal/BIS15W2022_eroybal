...
ui <- fluidPage(    
  ...
  sidebarPanel(
  selectInput("locomotion_type", " Select Locomotion Method:", choices=unique(homerange$locomotion)), hr(),
  ...
  output$locomotionPlot <- renderPlot({
    ...
    homerange %>% 
    filter(locomotion == input$locomotion_type) %>% 
    ggplot(aes(x=log10.hra)) + 
    geom_density(color="black", fill="red", alpha=0.6) +
      theme_light() +
      labs(title = 'Homerange Sizes by Locomotion Type',
           x = 'log10 of Homerange',
           y = 'Density')
  })
  }
