...
NA stands for Not Applicable and the summarize function will return only an NA value if one is present in the data being used.
```{r}
...
msleep %>%
  ...
  summarize(mean_brain = mean(brainwt, na.rm = TRUE),
            total = n())
```
Cetacea has a NaN value which is different from an NA value.
